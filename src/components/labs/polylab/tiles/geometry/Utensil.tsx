...
export const Utensil = memo(function Utensil({
  ...
  if (utensilType === 'protractor') {
    ...
    for (let deg = 0; deg <= 180; deg += 10) {
      ...
      angleTicks.push(
        <Line key={`tick_${deg}`} points={[x1, y1, x2, y2]} stroke="#1E293B" strokeWidth={isMajor ? 1.5 : 1} />
      );
  ...
  if (utensilType === 'compass') {
    const cx = width / 2;
    return (
      <TileShell id={id} x={x} y={y} rotation={rotation} width={120} height={140} isLocked={isLocked} isSelected={isSelected}>
        <Group>
          {/* Top Pivot Knob */}
  ...
  return (
    <TileShell id={id} x={x} y={y} rotation={rotation} width={rulerW} height={rulerH} isLocked={isLocked} isSelected={isSelected}>
      <Group>
        <Rect
          x={0}
          y={0}
        ...
        />
        {mmTicks}
        <Text x={rulerW - 30} y={26} text="cm" fontSize={9} fontStyle="bold" fill="#854D0E" />
      </Group>
    </TileShell>
  );
});
